...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ВРЕМЕННЫЙ: Тестирование iTunes API для обогащения дат релизов
  app.get('/api/test-itunes-dates', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await storage.getUser(userId);
      if (!user?.isAdmin) {
      ...
      console.log('🧪 Начинаем тестирование iTunes API для поиска дат релизов...');
      
      // Получаем 15 релизов без дат
      const releasesQuery = `
        SELECT 
          r.id,
      ...
      const releasesWithoutDates = await db.execute(sql.raw(releasesQuery));
      
      console.log(`📊 Найдено релизов без дат: ${releasesWithoutDates.rows.length}`);
      
      let foundDates = 0;
      let notFound = 0;
      const results = [];
      
      for (const release of releasesWithoutDates.rows) {
        console.log(`\n🔍 Поиск: ${String(release.artist_name)} - ${String(release.release_title)}`);
        
        try {
          const result = await musicAPI.findReleaseDate(String(release.artist_name), String(release.release_title));
          ...
          if (result) {
            ...
            results.push({
              id: release.id,
              artist: String(release.artist_name),
              title: String(release.release_title),
              foundDate: result,
              status: 'found'
            });
          } else {
          ...
          console.log(`⚠️ Ошибка поиска: ${error instanceof Error ? error.message : String(error)}`);
          notFound++;
          results.push({
            id: release.id,
            artist: String(release.artist_name),
      ...
      const effectiveness = Math.round(foundDates / (foundDates + notFound) * 100);
      
      console.log(`\n📈 РЕЗУЛЬТАТЫ ТЕСТИРОВАНИЯ:`);
      console.log(`✅ Найдено дат: ${foundDates}`);
      console.log(`❌ Не найдено: ${notFound}`);
      console.log(`📊 Эффективность: ${effectiveness}%`);
      
      res.json({
        summary: {
          total: foundDates + notFound,
          foundDates,
          notFound,
      ...
      console.error("Error testing iTunes API:", error);
      res.status(500).json({ message: "Failed to test iTunes API" });
    }
  });
